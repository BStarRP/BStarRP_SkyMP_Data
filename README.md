...
Public repository for **BStarRP SkyMP Launcher** patch content (mods + SkyrimPlatform data). The launcher fetches the **`patch-*.zip` release asset** (which contains only the contents of `Data/` + manifest) and installs it into the game's `Data` folder.
...
Push changes to `Data/` on `main` using [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) in your commit messages. The **Auto release on patch change** workflow sets the version from your commits, then **Build patch** attaches the zip.
...
- Files from repo `Data/` (installed into the game's Data folder by the launcher)
